---
title: "Week 1 Worklog"
date: 2026-04-24
weight: 1
chapter: false
pre: " <b> 1.1. </b> "

---

### Week 1 Objectives:

* Get acquainted with team members, create an AWS account, and get familiar with AWS architecture.

### Tasks to be deployed this week:

| Day | Task | Start Date | Completion Date | Reference Materials |
|:---:|:------------------------------------------------------------------------------------------------------------:|:------------:|:---------------:|:--------------------------------------------------------------------------------------------:|
| Mon | Create an AWS account and review the spending plan.                                                          | 2026/04/20   | 2026/04/20      | <https://000001.awsstudygroup.com/>                                                          |
| Tue | Perform tasks to earn enough 200 credits.                                                                    | 2026/04/21   | 2026/04/21      | https://000001.awsstudygroup.com/3-chi%E1%BA%BFn-l%C6%B0%E1%BB%A3c-nh%E1%BA%ADn-%C4%91%E1%BB%A7-200-credit/|
| Wed | Watch YouTube videos about module 1 to prepare relevant knowledge.                                           | 2026/04/22   | 2026/04/22      | https://www.youtube.com/watch?v=HxYZAK1coOI&list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i&index=4 |
| Thu | Perform related lab practices of module 1.                                                                   | 2026/04/23   | 2026/04/23      | https://www.youtube.com/watch?v=waR5S_lljrk&list=PLahN4TLWtox2a3vElknwzU_urND8hLn1i&index=11 |
| Fri | Write worklog and report.                                                                                    | 2026/04/24   | 2026/04/24      |                                                                                              |

### Results achieved in week 1:

* **General results:** Completed AWS account registration, performed tasks to earn enough 200 credits, and preliminarily acquired knowledge about module 1.
* **Weekly evaluation:**
 * Well completed the set targets.
 * Got familiar with the new working environment.
 * Got acquainted with people in the team.
* **Details of implementation results:**
  * **April 20, 2026:** AWS account has been confirmed and is active.
  * **April 21, 2026:** Earned enough 200 credits from tasks.
  * **April 22, 2026:** Mastered preliminary knowledge about module 1.
  * **April 23, 2026:** Successfully performed the first lab session.
  * **April 24, 2026:** Wrote worklog and submitted the first weekly report.
